...
urls = urls[::-1]
url_list = urls[3:5] # <-- Change the range to assign the pages to crawl
...
for url in url_list:
    ...
    for url2 in title_list:
        ...
        if info != []:
            ...
            except_list.append(url2)
    page -= 1
with open('3_5_content.json', 'w', encoding='utf-8') as file:
    json.dump(content_list, file, ensure_ascii= False)
with open('3_5_comments.json', 'w', encoding='utf-8') as file:
    json.dump(comments_list, file, ensure_ascii= False)
with open('3_5_log.json', 'w', encoding='utf-8') as file:
